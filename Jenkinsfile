pipeline {
    ...
    environment {
          ...
          ECR_REPO_NAME = 'food-delivery-restaurant-service'  // Replace with your ECR repository name
          IMAGE_TAG = "latest"
          WORKSPACE = "/var/lib/jenkins/workspace/food-delivery-restaurant-service"
    }
    stages {
        stage('Clone Repo to Build') {
            steps {
                git url: 'https://github.com/Topsan2002/food-delivery-restaurant-service.git',
                    branch:'master'
            }
        }
        ...
        stage('Deploy or Run Docker Container') {
            ...
            steps {
                script {
                    ...
                    def serviceName = "food-delivery-restaurant-service"
